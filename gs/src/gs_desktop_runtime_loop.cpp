...
#include "gs_desktop_render_helpers.h"
#include "gs_desktop_run_loop.h"
#include "gs_desktop_runtime.h"
#include "gs_desktop_runtime_sink.h"
    ...
    while (true)
    {
        ...
        do
        {
            ...
            const SessionEventPipelineDispatch dispatch = {
                ...
                {
                    logInvalidDesktopSessionEvent(invalid_event, videoFrameAssembler.currentFrameIndex());
                },
                [](const gs::core::SessionEvent&)
                {
                },
...
void signalHandler(int signal)
{
    if (signal == SIGTERM || signal == SIGINT)
    {
        ...
        cleanupDesktopSingleInstancePidFile();
        exitApp();
    }
}
...
int runDesktopRuntimeLoop(char* argv[])
{
    ...
    initializeDesktopOsd();

    Clock::time_point last_stats_tp = Clock::now();
    Clock::time_point last_tp = Clock::now();

    registerDesktopRenderCallback(config, argv);
    ...
    while (true)
    {
        processDesktopFrameTick(video_frame_count, last_stats_tp, last_tp, io);
    }
