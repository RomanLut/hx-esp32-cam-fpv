    ...
    while (true)
    {
        ...
        if (Clock::now() - last_stats_tp >= std::chrono::milliseconds(1000))
        {
            ...
            s_noPing = link_status.no_ping;
            {
                ...
                s_gs_stats.stabilizationFeaturesLastMS = static_cast<int>(stabilization_stats.feature_last_ms);
                s_gs_stats.stabilizationFeaturesMaxMS = static_cast<int>(stabilization_stats.feature_max_ms);
                s_gs_stats.stabilizationMotionLastMS = static_cast<int>(stabilization_stats.motion_last_ms);
                s_gs_stats.stabilizationMotionMaxMS = static_cast<int>(stabilization_stats.motion_max_ms);
    ...
    auto render_callback = [&config, argv]
    {
        ...
        ImGui::Begin("fullscreen", NULL, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoFocusOnAppearing);
        {
            ...
            if (s_imageStabilizationState.debug)
            {
                ...
                const int frame_h = static_cast<int>(frame_res.y);
                             ...
                             ImGuiIO& io)
{
    ...
    if (count == 0)
    {
        //Wait reasonably big interval to make sure we do no swap and stuck in vsynk before new frame is produced
