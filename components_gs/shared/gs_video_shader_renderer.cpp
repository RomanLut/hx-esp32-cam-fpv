...
constexpr char kFastPostFragmentShader[] = R"glsl(
#ifdef GL_FRAGMENT_PRECISION_HIGH
precision highp float;
#else
precision mediump float;
...
vec3 applyDeblocking(vec2 sample_coord, vec3 color)
{
    ...
    if (block_x < 0.5 || block_x > 6.5)
    {
        ...
        if (inFrame(p1_coord) && inFrame(q1_coord))
        {
...
constexpr char kLensPostFragmentShader[] = R"glsl(
#ifdef GL_FRAGMENT_PRECISION_HIGH
precision highp float;
#else
precision mediump float;
...
void VideoShaderRenderer::release()
{
    ...
    if (m_fast_post_program != 0)
    {
        glDeleteProgram(m_fast_post_program);
        m_fast_post_program = 0;
    }
    if (m_lens_post_program != 0)
    {
        glDeleteProgram(m_lens_post_program);
        m_lens_post_program = 0;
    }
    m_fast_post_failed = false;
    m_lens_post_failed = false;
}
                               ...
                               const VideoPostprocessingParams& postprocessing_params)
{
    ...
    const bool postprocessing_requested = postprocessing_params.deblocking_strength > 0.0f ||
                                          postprocessing_params.dithering_strength > 0.0f;
    const bool postprocessing_enabled = postprocessing_requested &&
                                        !(lens_enabled ? m_lens_post_failed : m_fast_post_failed);
    bool active_postprocessing = postprocessing_enabled;
    GLuint* program_slot = lens_enabled
        ? (postprocessing_enabled ? &m_lens_post_program : &m_lens_program)
        : (postprocessing_enabled ? &m_fast_post_program : &m_fast_program);
    if (*program_slot == 0)
    {
        const char* fragment_shader = lens_enabled
            ? (postprocessing_enabled ? kLensPostFragmentShader : kLensFragmentShader)
            : (postprocessing_enabled ? kFastPostFragmentShader : kFastFragmentShader);
        *program_slot = createProgram(fragment_shader);
        if (*program_slot == 0 && postprocessing_enabled)
        {
            LOGW("Video postprocessing shader unavailable, falling back to base video shader");
            if (lens_enabled)
            {
                m_lens_post_failed = true;
            }
            else
            {
                m_fast_post_failed = true;
            }
            active_postprocessing = false;
            program_slot = lens_enabled ? &m_lens_program : &m_fast_program;
            if (*program_slot == 0)
            {
    ...
    const GLuint program = *program_slot;
    ...
    glUniform1f(glGetUniformLocation(program, "uStabilizationEnabled"), stabilization_enabled ? 1.0f : 0.0f);
    ...
    if (active_postprocessing)
    {
        ...
        if (!s_logged_postprocessing_active)
        {
            ...
            LOGI("Video postprocessing shader active deblock=({:.3f},{:.3f},{:.3f},{:.3f}) dither=({:.4f},{:.3f})",
